...
function CustomPromptForm({ onSubmit }: { onSubmit: (v: { tab: string; section: string; category: string; text: string }) => void }) {
  ...
  return (
    <form
      onSubmit={(e) => {
        e.preventDefault()
        if (!text.trim()) return
        onSubmit({ tab: 'Custom', section: 'My Ideas', category: category.trim() || 'General', text: text.trim() })
      }}
      className="space-y-4"
    >
      <div className="space-y-2">
        <label className="block text-sm text-slate-600 dark:text-slate-300">Kategori</label>
        <input
          value={category}
          onChange={(e) => setCategory(e.target.value)}
          placeholder="Fx: My Ideas"
      ...
      <div className="space-y-2">
        <label className="block text-sm text-slate-600 dark:text-slate-300">Prompt</label>
        <textarea
          value={text}
          onChange={(e) => setText(e.target.value)}
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          className="px-4 py-2 rounded-2xl bg-amber-500 text-white hover:bg-amber-400 dark:hover:bg-amber-400 active:scale-[0.99]"
        >
          Gem
        </button>
      </div>
